...
/*
            ...
            {
                s_KEYB_EVENT * v = (s_KEYB_EVENT *)pValue;
                tasks[i].event.key.code = v->code;
                tasks[i].event.key.isLong = v->isLong;
            }
            if (type == LED_EVENT){
                s_LED_EVENT * v = (s_LED_EVENT *)pValue;
                tasks[i].event.led.state = v->state;
                tasks[i].event.led.delay = v->delay;
            }
            if (type == DISPLAY_EVENT){
                s_DISPLAY_EVENT * v = (s_DISPLAY_EVENT *)pValue;
                tasks[i].event.display.index = v->index;
                tasks[i].event.display.value = v->value;
            }
            */
        }
    }
